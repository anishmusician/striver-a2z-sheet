      ...
      <div className="bg-[var(--surface-1)] border border-[var(--surface-border-muted)] rounded-xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="table-auto w-full min-w-[850px] font-firaSans divide-y divide-[var(--surface-border-muted)]">
            ...
            <thead className="text-xs font-semibold leading-snug text-[var(--sheet-header-text)] bg-[var(--surface-1)] h-12 select-none">
              <tr>
            ...
            <tbody className="divide-y divide-[var(--surface-border-muted)] bg-[var(--surface-1)]">
              {problems.map((p) => {
                ...
                }[p.difficulty] || 'text-slate-400 bg-slate-800 border-slate-700';

                return (
                  <tr
                    key={p.id}
                    className={`transition-colors hover:bg-[var(--base-bg-hover)] ${
                      isSolved ? 'bg-[#121213]/80' : ''
                    }`}
                  >
                    {/* Status Checkbox */}
                    ...
                    <td className="px-4 py-3 w-[30%] text-left align-middle">
                      <button
                        ...
                        className={`text-left text-sm font-medium transition-colors hover:text-[var(--brand)] hover:underline cursor-pointer line-clamp-2 ${
                          isSolved ? 'text-zinc-400 line-through decoration-zinc-600' : 'text-[var(--base-text-primary)]'
                        }`}
                      >
                        {p.title}
                      </button>
                    ...
                    <td className="w-[8%] text-center align-middle">
                      {p.plus ? (
                        <button
                          onClick={() => onOpenWorkspace(p)}
                          className="text-[var(--brand)] hover:text-orange-300 text-xs font-semibold cursor-pointer px-2.5 py-1 rounded hover:bg-orange-500/10 transition-colors"
                    ...
                    <td className="w-[10%] text-center align-middle">
                      {p.editorial && p.youtube ? (
                        <button
                          ...
                          className="inline-flex items-center justify-center p-1 rounded hover:bg-zinc-800 transition-colors cursor-pointer group"
                        ...
                        >
                          <svg width="22" height="16" viewBox="0 0 24 18" fill="none" className="text-red-500 group-hover:scale-110 transition-transform">
                            <rect width="24" height="18" rx="4" fill="#EA763F" fillOpacity="0.2"/>
                      ...
                      {hasResource ? (
                        <div className="flex justify-center items-center gap-2">
                          ...
                          {p.youtube ? (
                            <button
                              onClick={() => onOpenVideo(p.youtube!, p.title)}
                              className="text-zinc-400 hover:text-red-400 transition-colors p-1 rounded hover:bg-zinc-800 cursor-pointer"
                    ...
                    <td className="w-[12%] text-center align-middle">
                      {hasPractice ? (
                        <div className="flex justify-center items-center gap-2">
                          {/* LeetCode icon - only rendered if leetcode link exists */}
                          {p.leetcode ? (
                            <a
                              href={p.leetcode}
                              target="_blank"
                          ...
                          {/* GFG badge - only rendered if genuine GFG practice link exists */}
                          {p.gfg ? (
                            <a
                              href={p.gfg}
                              target="_blank"
                            ...
                            >
                              <span className="font-bold text-[10px] text-emerald-400 bg-emerald-500/10 px-1 py-0.5 rounded border border-emerald-500/30 hover:bg-emerald-500/20">
                                GFG
                              </span>
                            </a>
                          ) : null}
                        ...
                        >
                          <FileEdit
                            className={`w-4 h-4 transition-colors ${
                              hasNotes ? 'text-[var(--brand)]' : 'text-zinc-500 hover:text-zinc-300'
                            }`}
                          />
                        </button>
                      </div>
                    ...
                    <td className="w-[8%] text-center align-middle">
                      <div className="flex justify-center items-center">
                        ...
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={starred ? "fill-yellow-500 stroke-yellow-500" : "stroke-zinc-600 fill-zinc-800/80 hover:stroke-yellow-400"}>
                    ...
                    <td className="w-[9%] text-center align-middle">
                      <div className="flex justify-center">
                        <span className={`text-[11px] font-medium px-2.5 py-0.5 rounded-md border text-center min-w-[62px] ${difficultyColor}`}>
                          {p.difficulty}
                        </span>
                      </div>
                    </td>
